...
namespace WindowsFormsApp7
{
    public partial class EmanetKitapVerfrm : Form
    {
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("insert into sepet(barkodno,kitapadi,yazar,yayinevi,sayfasayısı,kitapsayisi,teslimtarihi,iadetarihi) values(@barkodno,@kitapadi,@yazar,@yayinevi,@sayfasayısı,@kitapsayisi,@teslimtarihi,@iadetarihi)",baglanti);
            ...
            baglanti.Close();
            MessageBox.Show("Kitap(lar)sepete eklendi", "Ekleme İşlemi Tamamlandı");
        ...
        private void btnTeslimEt_Click(object sender, EventArgs e)
        {
            ...
            {
                if (lblKitapSayi.Text != "" && int.Parse(lblKitapSayi.Text) <= 3 || lblKayitliKitapSayi.Text != "" && int.Parse(lblKayitliKitapSayi.Text) <= 3)
                {
                    if (txtTcAra.Text != "" && txtYas.Text != "" && txtTelefon.Text != "")
                    {
                        for(int i=0; i < dataGridView1.Rows.Count - 1; i++)
                        {
                            baglanti.Open();
                            SqlCommand komut = new SqlCommand("insert into emanetkitaplar(tc,adsoyad,yas,telefon,barkodno,kitapadi,yazar,yayinevi,sayfasayisi,kitapsayisi,teslimtarihi,iadetarihi)values(@tc,@adsoyad,@yas,@telefon,@barkodno,@kitapadi,@yazar,@yayinevi,@sayfasayisi,@kitapsayisi,@teslimtarihi,@iadetarihi)", baglanti);
            ...
            {
             MessageBox.Show("Önce sepete eklenmelidir!!","Uyarı");
         }      
            }
